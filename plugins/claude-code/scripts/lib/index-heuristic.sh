...
agent_brain_index_candidates() {
  ...
  # Skip very short or purely acknowledgement messages
  if [[ ${#user} -lt 12 ]]; then
    echo '[]'
    return 0
  fi
  ...
  local subj content signal

  # Tier 1: explicit stated preferences
  if echo "$user" | grep -qiE '\b(i prefer|i like|i love|i hate|i always|i never|my goal is|i am allergic|i want|i need)\b'; then
  ...
  # Tier 2: corrections ("no not that", "actually", "wrong", "instead X")
