...
2. **Agent-brain MCP** — search tenant skills before any Cursor native skill:
...
3. **Native fallback only when agent-brain has no relevant skill** — Cursor skills (`~/.cursor/skills`, project `.cursor/skills`) and the Skill tool / `available_skills` list.
