...
`~/.cursor/replay/manifest.json`, dispatches one subagent per conversation (or chunk
...
Run the extraction script with the Shell tool **before** loading the manifest.

**Script path** (use the first that exists):
1. `~/.cursor/scripts/extract_conversations.py` (installed plugin)
2. `plugins/cursor/scripts/extract_conversations.py` (local checkout)
...
Read `~/.cursor/replay/manifest.json`. When `--project` was used, the manifest contains
...
```bash
python3 ~/.cursor/scripts/extract_conversations.py --project agent-brain
python3 ~/.cursor/scripts/extract_conversations.py --list-projects
```
